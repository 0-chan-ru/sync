# Full proxying CyTube (sync) by NGINX
...
```nginx
server {
	server_name tv.you.domain;
	listen 80;

	include includes/everyserv; # you other options...
...
server {
	...
	include includes/everyserv; # SSL and etc...
...
```
or custom proxy settings for socket.io and etc:
### PRO_NGINX.conf
```nginx
server {
	server_name tv.you.domain;
...
server {
	...
	location /socket.io/ {
		...
		proxy_pass http://127.0.0.1:8021/socket.io/;
		proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
		proxy_set_header X-Forwarded-Proto $scheme;
		proxy_set_header Host $http_host;
	}
...
html-template:
  title: you tv'
  description: 'you TV service powered by CyTube'
